...
def test_expand_workflow_dependencies_linear() -> None:
    ...
    config_b = AgentConfig(
        id="agent-b", name="B", persona="p", greeting="g", workflow_dependencies=["agent-c"]
    )
...
def test_expand_workflow_dependencies_diamond() -> None:
    ...
    dependency_configs = {"agent-b": config_b, "agent-c": config_c, "agent-d": config_d}
...
def test_expand_workflow_dependencies_cycle_safety() -> None:
    ...
    config_b = AgentConfig(
        id="agent-b", name="B", persona="p", greeting="g", workflow_dependencies=["agent-a"]
    )

    dependency_configs = {"agent-a": config_a, "agent-b": config_b}
